...
| Mahsuplaşma (12) | pencere açılışı ve ekstre taslağı · tek açık pencere kuralı · **kapsam** (yalnız USD seçilince altın ve diğer kurlar dokunulmaz) · mutabakat (eşit / farklı) · **altın teklifi ve onayı** (rafineri borçluyken teklif, Kanzasset borçluyken teklif edilemez) · **kısmi tutar** (girilen kadarı kapanır, kalanı cari hesapta durur; tutar bacağın tamamını aşamaz) · altın bacağının kasa talimatıyla kapanması · para bacağı ve cari hesabın kapanması · kesim saatinin bir kez tetiklenmesi |
