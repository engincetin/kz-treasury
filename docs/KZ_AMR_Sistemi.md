...
| **K2 Fiyat** (01) | Rafineri fiyatının ve müşteri fiyatının sağlığı | soket durumu (ayarlar K11'de) · fiyat zinciri (rafineri alış / satış → müşteri satar / alır, marj, komisyon) · son 10 tick · müşteri işlemlerinin durumu (açık / durdu ve sebebi) | **Müşteri işlemlerini durdur / başlat** (gerekçeli) | bayat fiyat ya da `tradable=false` → müşteri tarafı otomatik durur · yeniden bağlanma · `seq` boşluğunda yeniden abonelik |
