...
const OPS: Record<string, Op> = {
  ...
  "GET /api/documents": { summary: "K12 belgelerin Kanzasset kopyası (künye listesi)", description: "rafineri belgeleri olayla gelen numaradan çekilir, sha256 özeti yeniden hesaplanır (hash_ok), anahtar verilmişse imza doğrulanır (signature_ok) ve saklanır.", returns: "{ count, signature_checked, items[] }" },
  "GET /api/documents/:id": { summary: "K12 belgeyi kendi kaydımızdan getir (yoksa rafineriden çekip saklar)", returns: "{ meta (hash_ok, signature_ok, source dahil), content }" },
  "GET /api/documents/:id/pdf": { summary: "K12 belgenin PDF'i (rafineriden imzalı istekle çekilir, aynen aktarılır)" },
  "POST /api/documents/sync": { summary: "K12 geriye dönük eşitleme: emir, kasa, teslimat, rafinasyon ve mahsuplaşma kayıtlarındaki belge numaralarını tara, eksikleri çek", returns: "{ fetched, failed[], count }" },
